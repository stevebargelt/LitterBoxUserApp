...
struct imagePicker: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
        ...
        func uploadImageToFireBase(image: UIImage) {
            ...
            storage.child("catimages/\(parent.cat.id!)").putData(image.jpegData(compressionQuality: 0.42)!, metadata: metadata) { (metadata, error) in
        ...
        func downloadImageFromFirebase() {
            ...
            storage.child("catimages/\(parent.cat.id!)").downloadURL { (url, error) in
                if error != nil {
                    // Handle any errors
                    print((error?.localizedDescription)!)
                    return
        ...
        func listOfImageFile() {
            let storageReference = Storage.storage().reference().child("catimages/\(parent.cat.id!)")
